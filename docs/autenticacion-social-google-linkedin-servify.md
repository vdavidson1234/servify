...
sequenceDiagram
    ...
    alt Identidad existente
        ...
        alt Usuario existente
            ...
            API->>Store: Crea perfil inicial con nombre/foto del proveedor
...
Para el MVP, si el proveedor devuelve nombre y foto, el backend los usa para crear un perfil inicial. La foto no es obligatoria para que el perfil quede completo; si mas adelante se exige edad/ubicacion reales desde onboarding, conviene cambiar este perfil inicial para que quede incompleto hasta que el usuario confirme esos datos.
...
prestadorConPerfilCompletoSinFotoPuedeCrearPublicacion
```
...
El usuario creado por Google persiste perfil inicial y puede crear publicacion en el MVP
...
La foto de perfil es opcional para completar perfil y publicar
```
